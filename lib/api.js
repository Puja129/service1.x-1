...
class ECApi extends ECCommon {
	...
	*/
	...
	let _debug=this._debug, _this=this;
	...
	return new Promise((reso,reje)=>{		
	    ...
	    */
	    ...
	    _debug(`${new Date()} EC: ${_this.options['info']['id']} req.url===("/"+process.env.BASE+"/health/memory") ${req.url===("/"+process.env.BASE+"/health/memory")}`);

	    if (req.url===("/"+process.env.BASE+"/health/memory")){
		
		let health=new ECHealthApi(this._options,this._debug);
		    
		_debug(`${new Date()} EC: ${_this.options['info']['id']} checkMemory called`);
		    
		if (!health.init(req,res)) {
		    return reje({req:req,res:res,code:501});	    
		}

		_debug(`${new Date()} EC: ${_this.options['info']['id']} health.init called`); 
		return health.checkMemory().then(obj=>{
		    _debug(`${new Date()} EC: ${_this.options['info']['id']} checkMemory passed`); 
