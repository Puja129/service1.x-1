...
class ECApi extends ECCommon {
	...
	*/
	...
	return new Promise((reso,reje)=>{		
	    ...
	    */
	    _debug(`${new Date()} EC: ${process.env.ZONE} req.url===/${_this._options["info"]["pxy_ver"]}: ${req.url==="/"+_this._options["info"]["pxy_ver"]}`);
		    
	    if (req.url.indexOf(`/${_this._options["info"]["pxy_ver"]}`)>-1){
	       return reso({data:"proxy called",code:"200"});
	    }
