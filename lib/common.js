...
class ECCommon extends EventEmitter {
	...
	return new Promise((reso,reje)=>{	    
		...
		ls.on('close', (code) => {
 		  _dbg(`${new Date()} EC: ${process.env.EC_SVC_ID} child process (cmd) exited with code ${code}`);
		});
	});
    }
	...
	return new Promise((reso,reje)=>{	    
	  ...
	  ls.on('close', (code) => {	
            _dbg(`${new Date()} EC: ${process.env.EC_SVC_ID} child process (_getRefHash) exited with code ${code}`);
	  });
