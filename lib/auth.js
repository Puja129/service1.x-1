...
class RSAuth extends RSCommon {
	...
	_dbg(`${new Date()} EC: ${options["info"]["id"]} _oAuthTokenValidation c: ${JSON.stringify(c)} a: ${JSON.stringify(a)} f: ${JSON.stringify(f)}`);
		...
		_dbg(`${new Date()} EC: ${options["info"]["id"]} _oAuthTokenValidation > _request > _body: ${JSON.stringify(_body)}`);
