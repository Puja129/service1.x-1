...
class RSAuth extends RSCommon {
    ...
    //let _this=this;
    _getRefHash(hsh) {
	return _this.cmd('agent', ['-hsh', '-smp'], { env: {...process.env, EC_PPS: hsh }});
    }
	...
	return new Promise((reso,reje)=>{
	    ...
	    _dbg(`${new Date()} EC: ${process.env.EC_SVC_ID} validate > conf ${JSON.stringify(conf)}`);
	    //get auth info
