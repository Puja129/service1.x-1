...
class ECUserApi extends RSAuth {
	...
	try{
		...
		_this._debug(`${new Date()} EC: ${_this._options['info']['id']} ECUserApi > init > _o: ${this._o}  (EC User API)`);
	    
	    this._p=req.headers[PX_ZONE.toLowerCase()];

	} catch(e){
	    _this._debug(`${new Date()} EC: ${_this._options['info']['id']} ECUserApi > init > e: ${e}  (EC User API)`);
	    return false;
	}
